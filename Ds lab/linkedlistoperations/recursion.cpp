...
public:
    ...
    void reverse_list();
    node* recursive_insertion(node* head, int ele);
};
node* first=NULL;//global variable
//Holds the address of the first node
node* node :: recursive_insertion(node* head, int ele)
{
    ...
    {
        head->next = recursive_insertion(head->next, ele);
        cout << "kfah";
    }
    return head;

}
void node :: recursive(node* head)
{
    node* temp = head;
    if(temp != NULL)
    {
...
int main()
{
    ...
    do
    {
         ...
         case 'h':
            list1.insert_end();
            list1.recursive_insertion(first, 100);
            break;
         case 'i':
            list1.recursive(first);
            break;
         case 'j':
            break;
         default:
